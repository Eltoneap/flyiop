...
- **Nota (18/07/2026, atualizada em 21/07/2026):** a regra "em conflito, o VALIDACAO-CRUZADA prevalece" foi revogada em 21/07/2026 (ver nota de autoridade no topo do `PLAN-VALIDACAO-CRUZADA.md`); nenhum arquivo tem mais autoridade para reverter escopo sozinho, só decisão explícita no chat de planejamento. Nessa revisão, a confirmação por fonte independente (Google Flights via `fast-flights`) foi **reaprovada com escopo estreito**, como parte do A5 do `ROADMAP-AUDITORIA.md` (não como etapa própria) — ver as 4 regras registradas lá. Esta Etapa 7 (confirmação na 2ª consulta, mesma fonte Travelpayouts) permanece exatamente como está e independe dessa decisão.
