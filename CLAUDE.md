...
- Cada perna tem teto de preço editável, status (monitorando/comprada) e campo de observações livre (localizador, horário) preenchido depois da compra. **Fonte de verdade do teto: `settings.weekend_default_ceiling`** (teto padrão do usuário, editável no painel em Compras), com override opcional por perna em `weekend_leg_user_state.price_ceiling` — a view `weekend_leg_effective` resolve os dois e é de onde painel e robô leem. O valor do padrão muda conforme a calibração avança; não fixar número aqui.
...
- [ ] Recalibrar o teto padrão (`settings.weekend_default_ceiling`) assim que houver preços reais de ida e volta separados
